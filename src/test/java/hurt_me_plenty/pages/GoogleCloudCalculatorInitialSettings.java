...
import hurt_me_plenty.service.GoogleCalculatorAbstractContainer;
...
public class GoogleCloudCalculatorInitialSettings extends GoogleCalculatorAbstractContainer {
    ...
    private static final String XPATH_FOR_MACHINE_CLASS = "//md-select[@placeholder='VM Class']";
    private static final String XPATH_FOR_MACHINE_CLASS_ESTIMATE = "//div[@class='md-list-item-text ng-binding' and contains (text(),'regular')]";
    private static final String XPATH_FOR_INSTANCE_TYPE_ESTIMATE = "//div[@class='md-list-item-text ng-binding cpc-cart-multiline flex' and contains (text(),'n1-standard-8')]";
    private static final String XPATH_FOR_LOCATION_ESTIMATE = "//div[@class='md-list-item-text ng-binding' and contains (text(),'Frankfurt')]";
    private static final String XPATH_FOR_LOCAL_SSD_ESTIMATE = "//div[@class='md-list-item-text ng-binding cpc-cart-multiline flex' and contains (text(),'Local SSD: 4x375')]";
    private static final String XPATH_FOR_COMMITTED_USAGE_ESTIMATE = "//div[@class='md-list-item-text ng-binding' and contains(text(),'1 Year')]";
    private static final String XPATH_FOR_TOTAL_COST_AUTOMATED = "//h2[@class='md-title']//b[@class='ng-binding' and contains(text(),'Total Estimated Cost')]";
    private static final String XPATH_FOR_TOTAL_COST_MANUAL = "//*[@id=\"compute\"]/md-list[2]/md-list-item[9]";
    private static final String XPATH_FOR_SPECIAL_LOCATOR_TO_RESET_SETTINGS_SERIES_E2 = "//md-option[@value='e2']";
    ...
    @FindBy(xpath = XPATH_FOR_MACHINE_CLASS)
    private WebElement machineClass;

    @FindBy(xpath = XPATH_FOR_MACHINE_CLASS_ESTIMATE)
    private WebElement machineClassEstimate;

    @FindBy(xpath = XPATH_FOR_INSTANCE_TYPE_ESTIMATE)
    private WebElement instanceTypeEstimate;  //ex machineType

    @FindBy(xpath = XPATH_FOR_LOCATION_ESTIMATE)
    private WebElement locationEstimate;

    @FindBy(xpath = XPATH_FOR_LOCAL_SSD_ESTIMATE)
    private WebElement localSSDEstimate;

    @FindBy(xpath = XPATH_FOR_COMMITTED_USAGE_ESTIMATE)
    private WebElement committedUsageEstimate;

    @FindBy(xpath = XPATH_FOR_TOTAL_COST_AUTOMATED)
    private WebElement totalCostAutomated;

    @FindBy(xpath = XPATH_FOR_TOTAL_COST_MANUAL)
    private WebElement totalCostManual;

    @FindBy(xpath = XPATH_FOR_SPECIAL_LOCATOR_TO_RESET_SETTINGS_SERIES_E2)
    private WebElement specialLocatorToResetSettingsSeriesE2;
    ...
    public GoogleCloudCalculatorInitialSettings framesChange() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_FRAME_1);
        DRIVER.switchTo().defaultContent();
        DRIVER.switchTo().frame(frame1);
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_FRAME_2);
        DRIVER.switchTo().frame(frame2);
        return this;
    }

    public GoogleCloudCalculatorInitialSettings setNumberOfInstances() {
    ...
    public String takeMachineClassEstimateText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_MACHINE_CLASS_ESTIMATE);
        return machineClassEstimate.getText();
    }

    public String takeMachineClassText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_MACHINE_CLASS);
        return machineClass.getText().toLowerCase(Locale.ROOT);
    }

    public String takeMachineTypeText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_MACHINE_TYPE);
        return machineType.getText().substring(0, 13);
    }

    public String takeInstanceTypeEstimateText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_INSTANCE_TYPE_ESTIMATE);
        return instanceTypeEstimate.getText();
    }

    public String takeDataCenterLocationText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_DATACENTER_LOCATION);
        return datacenterLocation.getText().substring(0, 9);
    }

    public String takeLocationEstimateText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_LOCATION_ESTIMATE);
        return locationEstimate.getText();
    }

    public String takeLocalSSDEstimateText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_LOCAL_SSD_ESTIMATE);
        return localSSDEstimate.getText();
    }

    public String takeLocalSSDText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_LOCAL_SSD);
        return localSSD.getText().substring(0, 5);
    }

    public String takeCommittedUsageEstimateText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_COMMITTED_USAGE_ESTIMATE);
        return committedUsageEstimate.getText();
    }

    public String takeCommittedUsageText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_COMMITTED_USAGE);
        return committedUsage.getText();
    }

    public String takeTotalCostAutomatedText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_TOTAL_COST_AUTOMATED);
        return totalCostAutomated.getText();
    }

    //this exact method to click everything manually
    public String takeTotalCostManualText() {
        makeTheProgramWaitPresenceOfElementLocated(DRIVER, 40, XPATH_FOR_TOTAL_COST_MANUAL);
        return totalCostManual.getText();
    }

    public void resetAllTheSettingDefault() {
        makeTheProgramWaitElementToBeClickable(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_SERIES);
        series.click();
        makeTheProgramWaitElementToBeClickable(DRIVER, WAIT_TIMEOUT_SECONDS, XPATH_FOR_SPECIAL_LOCATOR_TO_RESET_SETTINGS_SERIES_E2);
        specialLocatorToResetSettingsSeriesE2.click();
    }
}
